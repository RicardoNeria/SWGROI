...
namespace SWGROI_Server.Security
{
    ...
    public static class SessionManager
    {
        ...
        private static void SetCookie(HttpListenerResponse res, string name, string value, bool httpOnly, bool secure, string sameSite = "Strict", int maxAgeSeconds = 60 * 60 * 8)
        {
        ...
        public static void EnsureSession(HttpListenerContext ctx)
        {
            ...
            var secure = IsHttps(req) || !IsLocalHost(req);
            SetCookie(res, "sessionid", s.Id, httpOnly: true, secure: secure, sameSite: "Strict");
            SetCookie(res, "csrftoken", s.Csrf, httpOnly: false, secure: secure, sameSite: "Strict");
        }
        ...
        public static void Destroy(HttpListenerContext ctx)
        {
            ...
            // Expira cookies
            res.Headers.Add("Set-Cookie", "sessionid=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict");
            res.Headers.Add("Set-Cookie", "csrftoken=; Path=/; Max-Age=0; SameSite=Strict");
        }
        ...
        public static (string user, string role)? GetUser(HttpListenerRequest req)
        {
            ...
            if (_sessions.TryGetValue(sid, out var s)) return (s.User, s.Role);
            return null;
        }
